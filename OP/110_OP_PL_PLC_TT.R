# ---
# title   : 110_OP_PL_PLC_TT
# version : 1.0
# author  : 2e consulting
# desc.   : [템프테이블] OP_PL_PLC 생성을 위한 템프성 테이블
# ---

# <<<<!---OP마트 구축 템플릿 공유를 위해 몇개의 샘플만 코드 작성---!>>>
...
for(i in 1:length(looping_ar)){
  ...
  PL_DRIVING_TABLE <- SparkR::sql(paste0("
      ...
      FROM        DM_PLC A1
      INNER JOIN  DW_INS_PROD A2
              ON  A1.PROD_CD           = A2.PROD_CD
             AND  A1.PROD_VER_CD       = A2.PROD_VER_CD
      INNER JOIN  DW_PL A3
              ON  A1.PONO              = A3.PONO
      INNER JOIN  DM_PTY A4
              ON  A1.BAS_YM            = A4.BAS_YM
             AND  A1.PLHD_INTG_CUST_NO = A4.INTG_CUST_NO
      INNER JOIN  DM_FC_BASIC A5
              ON  A1.BAS_YM            = A5.BAS_YM
             AND  A1.SVFC_NO           = A5.FC_NO
      ...
      WHERE       A1.BAS_YM            = '",v_BAS_YM1,"'                                 /*기준년월*/
        ...
        AND      !(A1.FST_PLC_DATE     < '2007-11-26'                  
                   AND A2.PROD_REPT_TYP_CD LIKE '%V%')                                   /*최초청약일이 2007-11-26 이전이며 상품보고서유형코드에 V가 포함된 계약 제외*/
        AND       A2.PL_PSB_TYP_CD     != '01'                                           /*약관대출불가 상품 제외*/
        AND       A3.PL_TYP_CD         = '01'                                            /*보험계약대출(APL제외)*/
        AND       A4.CUST_DETL_TYP_CD  IN ('11','12','22')                               /*고객상세유형코드. 내국인(11),외국인(12),개인사업자(22)*/
